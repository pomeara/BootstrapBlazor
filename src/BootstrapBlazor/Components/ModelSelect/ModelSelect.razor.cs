...
/// <summary>
/// Model dropdown for one provider — items carry per-model atoms (favourite star, ELO, tier,
/// state) via the base <see cref="ModelSelectorBase"/> filtering and auto-select policy.
/// </summary>
public sealed partial class ModelSelect
{
    /// <summary>Appends the input price to each item's text for paid models.</summary>
    [Parameter]
    ...
    protected override void OnParametersSet()
    {
        ...
        _items = CurrentModels
            .Select(m => new SelectedItem(m.Id, FormatModelDetail(m, ShowPricing)))
            .ToList();
    }
}
